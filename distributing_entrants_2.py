...
def priority_count(preferences_stud, speciality_col):
    priority_total = []
...
def Distribution(preferences_stud, preferences_spec, scores, distrib_stud, distrib_scores, speciality_col, quotas):
    ...
    # рейтинг абитуриентов (для каждого entr: ключи = специальность, значение = рейтинг)
    priority = priority_count(preferences_stud, speciality_col)
    ...
    while len(not_stud) != 0:
        ...
        if preferences_stud.loc[entr]["Entrants"][1] > preferences_stud[speciality_col].loc[entr].max():
            ...
            for spec in priority[entr].keys():

                if (priority[entr][spec] == preferences_stud.loc[entr]["Entrants"][1]):
